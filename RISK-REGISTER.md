...
**Status:** Resolved (final whole-branch review, 2026-09-07). `InMemoryAgentActionStore.claim()` is now a true insert-or-null: its lenient reclaim branch (`status !== 'needs_attention' && attemptCount > 0 → return existing`) was removed entirely, so `claim()` only ever creates a genuinely new row and returns `null` for any existing row regardless of status — matching the spec's original intent to *port* (not duplicate) the reclaim logic into `reclaimForRetry`, and matching `createDrizzleAgentActionStore.claim()`'s real semantics. The blast-radius concern below was checked for real (not assumed): `InMemoryAgentActionStore` has zero usage in `packages/adapter-example`; its only repo-wide consumers are `packages/core/test/agent-action-store-in-memory.test.ts` (already set `status` explicitly, unaffected) and `packages/core/test/agent-runner.test.ts` (two call sites, ~line 157/216, updated from `store.update(claimed.id, { attemptCount: 2 })` to `store.update(claimed.id, { status: 'draft_failed', attemptCount: 2 })` — more realistic anyway, matching what a real Drizzle store's row shape would actually be at that point). Full `packages/core` suite re-run after the change with no other regressions found.
**Filed:** 2026-09-06
...
**Blocking:** Was not blocking the CorpFlow automation-builder plan (parked during Task 2's review rather than reopening already-merged Task 1 work); resolved for real in the final whole-branch review before this branch was considered done.

---

## RISK-012: A bare `npm install` at the repo root fails in a genuinely fresh clone (monorepo workspace build-ordering)

**Status:** Resolved. The commit originally credited here, `e17f46f` (`--prefix ../core` in `adapter-corpflow`'s `prepare` script), was itself reverted once `v0.8.2` was properly subtree-split (see the 2026-09-07 Correction below) — the real, final fix is commit `18c2330` (`fix(adapter-corpflow): revert to git-tag devDependency pin, drop --prefix ../core`), which reverted `prepare` back to plain `"npm run build"` and pinned `adapter-corpflow`'s `@aria/core` devDependency to the real `core-v0.8.3` git tag, landing in the `v0.8.3` cut.
**Filed:** 2026-09-06
...
**Verified workaround (obsolete):** `npm install --ignore-scripts && npm run build && npm test` was verified against the `e17f46f`-era `--prefix ../core` fix. That fix was reverted in `v0.8.3` (see Status above and the 2026-09-07 Correction below), and the underlying `--prefix ../core` failure mode this workaround was needed for no longer exists in the current `prepare` script — this workaround is no longer applicable and should not be followed.
...
**Verified:** A real `npm install` against the actual pushed `github:twalibey/aria-core#adapter-corpflow-v0.8.0` / `#core-v0.8.0` tags (pre-fix) reproduces the exact TS7016/TS7006 crash from a scratch consumer project. Cloning this repo at the fix commit and installing it as a git dependency of a scratch consumer project (via `git+file://...#<tag>::path:<package>`, mimicking npm's real git-tag install mechanism) completes with zero errors post-fix, where it previously failed. Full workspace `build`/`test` (271/271 tests) still pass with no regression. **Correction (final whole-branch review, 2026-09-07):** the `typecheck` failure this entry originally described as "pre-existing... unrelated to this change" was wrong — `git show 3a48d9e:packages/core/src/agent-types.ts` confirms `reclaimForRetry` does not exist at this branch's base commit, so the failure (the `laxStore` fixture in `agent-runner.test.ts` not implementing the newly-required `reclaimForRetry` member) was branch-introduced by Task 1, not pre-existing. It has since been fixed by adding `reclaimForRetry: async () => null,` to that fixture.
